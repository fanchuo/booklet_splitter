        ...
        for booklet_name, pages in self.constituted_booklets.items():
            ...
            for rank in range(0, int(nb_pages / 2)):
                ...
                if rank % 2 == 0:
                    ...
                    blank.mergeRotatedScaledTranslatedPage(
                        page2, 90, scale, self.width, self.height / 2,
                        expand=True)
                else:
                    blank.mergeRotatedScaledTranslatedPage(
...
class PdfHandler(object):
    ...
    def __str__(self):
        return '[PdfHandler - numPages : {0} - width : {1} - height : {2}]' \
            .format(self.num_pages, self.width, self.height)
    ...
    def compute_booklets(
        ...
        return BookletsCollection(
            constituted_booklets, self.width, self.height)
    ...
    def _fill_booklets(
            ...
            cover: bool) -> Dict[str, List[PyPDF2.pdf.PageObject]]:
        """
        ...
        for s_booklet in booklets_to_process:
            ...
            for booklet_idx in range(0, s_booklet):
                if page_idx < 0 or page_idx >= self.num_pages:
                    pages.append(PyPDF2.pdf.PageObject.createBlankPage(
                        pdf=None, width=self.width, height=self.height))
                else:
